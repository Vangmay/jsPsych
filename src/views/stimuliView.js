...
var s2 = {
  ...
  on_start: async function getStimuli() {
    ...
    document.querySelector(
      "#jspsych-html-button-response-button-1 button"
    ).disabled = false;
  ...
  on_finish: function (data) {
    ...
    if (data.response == 0) {
      ...
      if (getCount() >= 3) jsPsych.addNodeToEndOfTimeline(s4);
      else jsPsych.addNodeToEndOfTimeline(s2);
    }
  },
};
...
function addRespFromButton(data, rt) {
  var accept = "rejected";
  if (data.response == 0) accept = "accepted";
...
function printResult(result) {
  ...
  document.querySelector(
    "#jspsych-image-button-response-button-0 button"
  ).disabled = false;
  document.querySelector(
    "#jspsych-image-button-response-button-1 button"
  ).disabled = false;
...
var s2_img = {
  ...
  button_html: () => {
    var html_btn0 =
      '<div class="btn-img"><button class="my-jspsych-btn">%choice%</button>';
    ...
    if (get_condition().show_score)
      ...
      html_btn0 +=
        '<div class="div-hint" id="hint_gen">It takes a few seconds for the Gen AI to create a new name. Thank you for your patience.</div>';
    html_btn0 += "</div>";

    var html_btn1 =
      '<div class="btn-img"><button class="my-jspsych-btn">%choice%</button>';
    html_btn1 +=
      '<div class="div-hint" id="hint_stop">I have made up my mind.</div></div>';
  ...
  prompt:
    '<div id="title_container" class="div-title"><p id="title" class="p-title">\t\t\t\t</p></div >',
  ...
  on_start: function () {
    ...
    div.innerHTML = html1;
    document
      .getElementsByClassName("jspsych-display-element")[0]
      .appendChild(div); //put the template on display

    // get actual data of components

    //score component
    var html_score =
      "Remaining points: " + globalThis.myResultModel.getCount() + "\n";
    ...
    if (get_condition().bank_position == "corner") {
      ...
      html_pool =
        '<p align="center"><b>Title Bank</b></p>' + html_pool.join("");
  ...
  on_load: async function () {
    ...
    globalThis.myResultModel
      .calTitle(0, [5000, 8000])
  ...
  on_finish: function (data) {
    ...
    globalThis.myResultModel.saveResult("", data.response, data.rt, startTime);
    ...
    if (globalThis.myResultModel.getCount() <= 0)
      ...
      if (data.response == 1) {
        //if subject choose to stop
        jsPsych.addNodeToEndOfTimeline(s2_choose);
      } else {
        //if subject choose to generate
        globalThis.myResultModel.addCount(); //add the times of generation, in this case, minus 2 score

        if (get_condition().isSlider)
          //update similarity if user wants new
...
function hint_hover() {
  document
    .querySelector("#jspsych-image-button-response-button-0 button")
  ...
  document
    .querySelector("#jspsych-image-button-response-button-1 button")
    .addEventListener("mouseover", () => {
...
var s2_choose = {
  ...
  button_html: [
    '<button class="jspsych-btn" style = "position:fixed; bottom: 20px;right:60px;">%choice%</button>',
  ],
  questions: [
    {
...
function canGo(radio, input) {
  ...
  title = input.value;
  var canGo =
    radio.checked & (title != "") & (title != "enter your own title here");
  if (canGo)
    document.querySelector(
...
var s3_own = {
    ...
    return [
      {
        ...
        name: "choice_own",
        options: [
          `Yes, I want to submit <i>${chosen}</i> for evaluation`,
          "No, I want to submit my own title",
        ],
      },
    ];
  },
  ...
  on_load() {
    var radio = document.querySelector(
      "#jspsych-survey-multi-choice-response-0-1"
    );
    var radio_yes = document.querySelector(
      "#jspsych-survey-multi-choice-response-0-0"
    );

    //input
    var input_html =
      '<input id="myTitle" placeholder="Enter your own title here" class="input-title">';
  ...
  on_finish: function (data) {
    ...
    //save results,don't use start time
    globalThis.myResultModel.saveResult(data.trial_type, response, data.rt, -1);

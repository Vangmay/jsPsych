  ...
  table.forEach((tt) => {
    if (tt[0] == target) {
      ...
      if (Math.abs(tt[2] - sim) < minDistance) {
        ...
        if (pool.indexOf(other_title) < 0 && newTitle.length <= 80) {
...
function fullscreenListener(event) {
  ...
  var notFull =
    window.screenTop + window.screenLeft || window.screenY + window.screeX; //ensure the functionality in old versions
  //console.log("top screen: " + window.screenTop + " left screen: " + window.screenLeft);

  if (userAgent.indexOf("Chrome") != -1)
    notFull =
      window.screenTop + window.screenLeft <= 0 ||
      window.screenY + window.screeX <= 0;
...
export { loadFile, getSimilar, fullscreenListener };
